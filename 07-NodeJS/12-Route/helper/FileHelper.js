 ...
 */
 ...
 const mkdirs= (target, permission='0755') =>{
     ...
     //주어진 경로가 절대경로 형식이라면 경로를 누적할 변수를 '/'부터 시작
     if(target.substring(0,1) == '/'){
         dir='/';
     }
 
     //윈도우의 경우 하드디스크 문자열을 구분하기 위해 ':'이 포함되어 있음
...
const initMulter= () =>{
    ...
    const multipart= multer({
        ...
        storage: multer.diskStorage({
          ...
          req: 요청 정보, file은 최종적으로 업로드된 결과 데이터가 저장되어있을 객체 */
          destination: (req, file, callback)=> {
            /**file파라미터의 형식
             * file={
                fieldname: 'myphoto',
                originmalname: '원본파일명.jpg',
                encoding: '7bit',
                mimetype: 'image/jpeg'
              }
             */
            ...
            /*업로드 정보에 백엔드의 업로드 파일 저장 폴더 위치를 추가함
            ...
            //multer객체에게 업로드 경로를 전달
            callback(null,file.upload_dir);
          },
          //업로드된 파일이 저장될 파일 이름을 결정함
          filename: (req, file, callback)=> {
            /*file파라미터는 앞의 과정을 통해 정보가 확장(추가)된 상태
            file= {
              filedname: 'myphoto',
              originalname: '원본파일명.jpg',
              encoding: '7bit',
              mimetype: 'image/jpeg',
              upload_dir: '업로드된 파일이 저장될 경로',
           ...
           // console.err(parseInt(eval(process.env.UPLOAD_MAX_SIZE)));
            
            //파일 원본 이름에서 확장자만 추출-> ex).png
            const extName= extname(file.originalname).toLowerCase();
            //파일이 저정될 이름(현재_시각의_timestamp+ 확장자)
            const saveName= new Date().getTime().toString()+ extName;
      
            //업로드 정보에 백엔드에 업로드되어 저장된 파일 이름을 추가함
            file.savename=saveName;
            //업로드된 파일이 저장된 최종 경로를 추가함
            ...
            //구성된 최종 업로드 정보를 클라이언트에게 응답결과로 돌려주기위해 req객체에게 추가
            if(req.file instanceof Array){
                //멀티업로드일 경우(req.file이 배열일 경우)
                req.file.push(file);
            }else{
                req.file= file;
            }
            //다음단계로 백엔드상에 저장할 파일 이름 전달.
...
const createThumbnail= file=> {
    //환경 설정 파일에서 썸네일 이미지 크기를 가져온 후 정수 배열로 반환함
    ...
    size.map( async (v, i)=> {
        ...
        //프론트엔드에게 전송될 정보에 'thumbnail'이라는 프로퍼티가 없다면 빈 json형태로 생성
        if(!file.hasOwnProperty('thumbnail')){
            file.thumbnail= {};
        }
        //썸네일 정보를 width를 key로 갖는 json형태로 추가하기 위해 key 이름 생성
        const key= v+ 'w';
